...
; Misc
Debug_Resize =		-1;$10					; resize routine. -1 stops resize routines from running
Max_Water_Hurt_Delay =	55					; number of safety frames before water hurts
Tick_Water_Hurt_Delay_A = 25					; number of frames before 1/5 of segment is consumed (assist)
Tick_Water_Hurt_Delay_N = 18					; number of frames before 1/5 of segment is consumed (normal)
Tick_Water_Hurt_Delay_H = 15					; number of frames before 1/5 of segment is consumed (hard)
Tick_Water_Hurt_Delay_M = 10					; number of frames before 1/5 of segment is consumed (maniac)
Enable_SplashScreen =		0
...
id_LevelSelect =					id(ptr_GM_LevelSelect)
id_Level =						id(ptr_GM_Level)
id_SegaScreen =					id(ptr_Sega_Screen)
id_TitleWithNotice =				id(ptr_GM_TitleWithNotice)
id_Title =						id(ptr_GM_Title)
id_Options = 						id(ptr_Options_Screen)
id_AssistWarning = 				id(ptr_AssistWarning_Screen)
id_DDZ_Phase2 =					id(ptr_DDZ_Phase2)			; MJ: phase 2 of final boss
id_DDZ_Ending =					id(ptr_DDZ_Ending)			; MJ: phase 2 ending scene
id_Thanks =						id(ptr_Thanks)				; MJ: thank you for playing screen
id_RedMiso =					id(ptr_RedMiso)				; MJ: Red Miso screen
id_Results =						id(ptr_Results)
id_EXMode =					id(ptr_EXMode)
id_Dialog =						id(ptr_Dialog)
...
VintID_Lag =						id(ptr_VInt_Lag)
VintID_Main =					id(ptr_VInt_Main)
VintID_Sega =					id(ptr_VInt_Sega)
VintID_Title =					id(ptr_VInt_Title)
VintID_Menu =					id(ptr_VInt_Menu)
VintID_Level =					id(ptr_VInt_Level)
VintID_TitleCard =				id(ptr_VInt_TitleCard)
VintID_Fade =					id(ptr_VInt_Fade)
; ---------------------------------------------------------------------------
; Levels
id_FDZ =				0
id_SCZ =				1
...
vram_window =				$E800			; window namespace
...
collision_restore_flags =		$25			; byte ; restore collision after hit (maybe it's used only bosses)
...
ArtTile_Sonic =					$680
ArtTile_HUD =					$6B4	;$6C2
ArtTile_Ring =					$7E4	;$6B4
ArtTile_Ring_Sparks =				$7EC	;$6BC
ArtTile_Powerups =				$518
ArtTile_Shield =				$79C
ArtTile_Shield_Sparks =				$7BB
ArtTile_DashDust =				$7F0
ArtTile_Pause =					$6CC	; MJ: the pause and death text will never show at the...
ArtTile_YouDied =				$6CC	; MJ: ...same time
; ---------------------------------------------------------------------------
...
VRAM_CreditsSprites	= $4000					; real-time sprite line art for credits
VRAM_CreditsArt		= (VRAM_CreditsSprites+$40)		; line art for credits
VRAM_CreditsText	= (VRAM_CreditsSprites-$5A0)		; credits text VRAM address
VRAM_CreditsMiso	= $7000					; miso logo (the small one)
VRAM_CreditsChars	= $7000					; sprite text art write location
VRAM_CreditsLargeChars	= $7000					; VRAM address where large art is to be loaded
VRAM_CreditsClouds	= $9400					; sprite cloud art
VRAM_CreditsFeal	= $AB00					; VRAM address for feal enemies (overwrites body-parts which don't show in the credits)
VRAM_CreditsTails	= VRAM_CreditsArt			; overwrite line art

; ===========================================================================
; TEMPORARY SONG ID MAPPING
; ===========================================================================

mus_Lust	EQU mus_Invincible
mus_Ending	EQU mus_Title
mus_Drowning	EQU mus_Invincible
