...
sub_2D028:
		...
		btst	#1,$2A(a0)		; NAT: Check if starpost is upside-down
		beq.s	.noflip			; if not, skip
		bset	#1,$2A(a1)		; flip the star
		addi.w	#$14*2,$32(a1)		; move the y-pos to be below the starpost as well
...
Save_Level_Data:
		...
		move.b	(Player_1+status).w,(Saved_status).w
		andi.b	#1,(Saved_status).w
		move.b	(Ring_count).w,(Saved_ring_count).w
		...
		move.w	(Target_water_level).w,(Saved_mean_water_level).w
		move.b	(Water_full_screen_flag).w,(Saved_water_full_screen_flag).w
		move.b	(GravityAngle).w,(Saved_GravityAngle).w
		rts
...
Load_Starpost_Settings:
		...
		move.b	(Saved_GravityAngle).w,(GravityAngle).w

		tst.b	(Water_flag).w
		beq.s	+
		move.w	(Saved_mean_water_level).w,(Mean_water_level).w
		move.w	(Mean_water_level).w,(Target_water_level).w
