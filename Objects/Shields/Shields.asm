...
Obj_Fire_Shield_Main:
		...
		bne.w	Obj_Fire_Shield_DestroyUnderwater	; If so, branch
...
Obj_Fire_Shield_Display:
		...
		cmpi.w	#2,(Current_zone_and_act).w		; yes??? (FDZ3 check)
		bne.s	.anim

		tst.b	(FDZ3Rain_Spawn).w
		beq.s	.anim
		subq.w	#1,$2E(a0)
		bmi.s	Obj_Fire_Shield_Destroy_Rain
		moveq	#$F,d0
...
Obj_Fire_Shield_Destroy_Rain:
		andi.b	#$8E,status_secondary(a2)	; Sets Status_Shield, Status_FireShield, Status_LtngShield, and Status_BublShield to 0
		bsr.s	Obj_Fire_Shield_Destroy
		samp	sfx_BossFlame
		lea	(ChildObjDat_DialogueFirebrandRadiusExplosion22).l,a2
...
Obj_Big_Insta_Shield:
		; Init
		move.l	#Map_BigInstaShield,mappings(a0)
		move.l	#DPLC_BigInstaShield,DPLC_Address(a0)			; Used by PLCLoad_Shields
		move.l	#ArtUnc_BigInstaShield>>1,Art_Address(a0)			; Used by PLCLoad_Shields
		move.b	#4,render_flags(a0)
		move.w	#$80,priority(a0)
...
.nothighpriority:
		move.w	#1,anim(a0)			; Clear anim and set prev_anim to 1
		st	LastLoadedDPLC(a0)		; Reset LastLoadedDPLC (used by PLCLoad_Shields)
		move.l	#Obj_Big_Insta_Shield_Main,(a0)

Obj_Big_Insta_Shield_Main:
		lea	(Player_1).w,a2
		btst	#Status_Invincible,status_secondary(a2) ; Is the player invincible?
		bne.w	Obj_Big_Insta_Shield_Return			; If so, return
		tst.w	(Enlarged_InstaShield).w
		beq.w	Obj_Big_Insta_Shield_Destroy
		move.w	x_pos(a2),x_pos(a0)		; Inherit player's x_pos
		move.w	y_pos(a2),y_pos(a0)		; Inherit player's y_pos
		move.b	status(a2),status(a0)		; Inherit status
		andi.b	#1,status(a0)			; Limit inheritance to 'orientation' bit
...
.nothighpriority:
		...
		move.w	#$80,priority(a0)		; Layer shield over player sprite
		cmpi.b	#7,mapping_frame(a0)		; Has it reached then end of its animation?
		bne.s	.overplayer			; If not, branch
		tst.b	double_jump_flag(a2)		; Is it in its attacking state?
		beq.s	.overplayer			; If not, branch
		move.b	#2,double_jump_flag(a2)		; Mark attack as over
...
Obj_Big_Insta_Shield_Destroy:
		andi.b	#$8E,status_secondary(a2)
		move.l	#Obj_Insta_Shield,(a0)

Obj_Big_Insta_Shield_Return:
		rts
...
Obj_BlueShield:
		...
		move.l	#ArtUnc_BlueShield>>1,Art_Address(a0)			; Used by PLCLoad_Shields
		move.b	#4,render_flags(a0)
		move.w	#$80,priority(a0)
		move.b	#$18,width_pixels(a0)
		move.b	#$18,height_pixels(a0)
...
.nothighpriority:
		move.w	#1,anim(a0)									; Clear anim and set prev_anim to 1
		st	LastLoadedDPLC(a0)								; Reset LastLoadedDPLC (used by PLCLoad_Shields)
		move.l	#Obj_BlueShield_Main,address(a0)

Obj_BlueShield_Main:
		lea	(Player_1).w,a2
		btst	#Status_Invincible,status_secondary(a2)
		bne.s	Obj_Big_Insta_Shield_Return
		cmpi.b	#id_Null,anim(a2)								; Is player in their 'blank' animation?
		beq.s	Obj_Big_Insta_Shield_Return					; If so, do not display and do not update variables
		btst	#Status_Shield,status_secondary(a2)
		beq.w	Obj_Bubble_Shield_Destroy
		move.w	x_pos(a2),x_pos(a0)
		move.w	y_pos(a2),y_pos(a0)
...
.normalgravity:
		andi.w	#drawing_mask,art_tile(a0)
		tst.w	art_tile(a2)
		bpl.s	Obj_BlueShield_Display
		ori.w	#high_priority,art_tile(a0)

Obj_BlueShield_Display:
...
loc_18868:
		...
		beq.w	Obj_Lightning_Shield_Spark_Delete
		move.w	x_pos(a1),d0
		move.w	d0,x_pos(a0)
		move.w	y_pos(a1),d1
		move.w	d1,y_pos(a0)
...
; ---------------------------------------------------------------------------
		...
		include "Objects/Shields/Object data/Map - Big Insta-Shield.asm"
		include "Objects/Shields/Object data/DPLC - Big Insta-Shield.asm"
