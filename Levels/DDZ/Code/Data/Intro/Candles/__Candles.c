...

{
	...
	KosMComp (Art.Data, Art.Size);
	if ((File = fopen ("Art.kosm", "wb")) == NULL)
	{
		FreeArt (Art);
		FreeMap (Map);
		FreePlc (Plc);
	...
	//	fseek (File, MapLoc, SEEK_SET);
		if (Map.List [Loc].Count != 1) { printf ("Warning, frame %d has more than one MAP entry\n", Loc); }
	//	fputc (Map.List [Loc].Count >> 8, File);
	//	fputc (Map.List [Loc].Count >> 0, File);
