...
{
	Return = FileSprites (PalBin, PalLine, PalCount,	Art, Map, Plc,	FALSE, SP_PLC_NONE, SP_SPRITES,	Direct);
...

{
	...
	for (int Loc = 0; Loc < Map.Size; Loc++)
	{
		...
		{
	int AdjX = ((Map.List [Loc].Piece [Pos].S&0xC)+4);
	int AdjY = ((Map.List [Loc].Piece [Pos].S&3)+1)<<2;
			fputc ((Map.List [Loc].Piece [Pos].Y+AdjY) >> 8, File);
			fputc ((Map.List [Loc].Piece [Pos].Y+AdjY), File);
		fputc (0x00, File);
		fputc (AdjY, File);
			fputc (0x00, File);
			...
			fputc ((Map.List [Loc].Piece [Pos].X+AdjX) >> 8, File);
			fputc ((Map.List [Loc].Piece [Pos].X+AdjX), File);
		fputc (0x00, File);
		fputc (AdjX, File);
		}
		int MapPoint = MapLoc;
