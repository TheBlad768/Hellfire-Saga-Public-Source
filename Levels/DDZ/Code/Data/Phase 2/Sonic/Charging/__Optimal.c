...

{
	...
	for (int Loc = 0; Loc < Map.Size; Loc++)
	{
		...
		{
			fputc ((Map.List [Loc].Piece [Pos].Y+0x00) >> 8, File);
			fputc ((Map.List [Loc].Piece [Pos].Y+0x00), File);
			fputc (0x00, File);
			fputc (Map.List [Loc].Piece [Pos].S, File);
			fputc (Map.List [Loc].Piece [Pos].V >> 8, File);
			fputc (Map.List [Loc].Piece [Pos].V, File);
			fputc ((Map.List [Loc].Piece [Pos].X+0x00) >> 8, File);
			fputc ((Map.List [Loc].Piece [Pos].X+0x00), File);
		}
		int MapPoint = MapLoc;
		MapLoc = ftell (File);
