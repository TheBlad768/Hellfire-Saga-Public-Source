...
loc_EC5A:
	...
	; yay for messy code - turns out this routine is a piece of shit
		move.b	GravityAngle.w,d0	; load gravity angle to d0
		add.b	#$40,d0			; test -$40 to $40
		bpl.s	.run			; if between those angles, run only

	; this algorithm negates the angle while preserving sign bit
		move.b	angle(a0),d0		; load angle to d0
		add.b	#$40,d0			; offset by $40 (1/4)
		neg.b	d0			; negate angle now
		sub.b	#$40,d0			; undo offset of $40 (1/4)
		move.b	d0,angle(a0)		; save as angle again
		bsr.s	.run
...
Player_Angle:
		move.w	d0,d3				; load first floor offset to d3
		move.b	(Secondary_Angle).w,d2		; load angle of second block
		cmp.w	d0,d1				; check which floor was higher
		ble.s	loc_ED5E			; branch if second one(?)

		move.b	(Primary_Angle).w,d2		; check angle of first block
		move.w	d1,d3				; load second floor offset to d3
		move.w	d0,d1

loc_ED5E:
		btst	#0,d2				; check bit0 of angle
		bne.s	loc_ED7A			; if set, branch
		move.b	d2,d0				; copy angle to d0
		sub.b	$26(a0),d0			; substract current angle
		bpl.s	loc_ED6E			; branch if result is positive
		neg.b	d0				; negate angle offset

loc_ED6E:
		cmpi.b	#$20,d0				; check if difference was >$20 degrees (1/8)
		bhs.s	loc_ED7A			; if so, run alternate code
		move.b	d2,$26(a0)			; save as new angle
		rts
; ---------------------------------------------------------------------------

loc_ED7A:
		move.b	$26(a0),d2			; load angle to d2
		addi.b	#$20,d2				; add $20 to it (1/8)
		andi.b	#$C0,d2				; keep only in range of $40 increments (1/4)
		move.b	d2,$26(a0)			; save as new angle
		rts
...
+		move.b	top_solid_bit(a0),d5		; NAT: added this code to fix the damn top solid collision for reverse gravity
...
ObjCheckCeilingDist_Rev:
		move.w	x_pos(a0),d3

ObjCheckCeilingDist2:
		move.w	y_pos(a0),d2
		moveq	#0,d0
		move.b	y_radius(a0),d0
		ext.w	d0
