...
Render_HUD:
	; render HUD
		tst.b	HUD_State.w				; check HUD state
		beq.w	.Done					; branch if disabled
		bmi.s	.getout					; branch if HUD is on its way out

		tst.b	Level_end_flag.w			; is level ending?
		beq.s	.noend					; if not, branch
		st	HUD_State.w				; set as going out
		bra.s	.getout					; run dat code

.noend
		cmp.b	#2,HUD_State.w				; check current state
		bgt.s	.render					; if 3, only render
		beq.s	.getin					; branch if 2

	; init
		move.w	#$10,HUD_Xpos.w				; set default x-pos
		move.w	#$800,HUD_Xvel.w			; set x-velocity
		addq.b	#1,HUD_State.w				; moving done
; ---------------------------------------------------------------------------

	; decelerate right
.getin
		sub.w	#$40,HUD_Xvel.w				; speed HUD up towards left
		bpl.s	.nostop					; branch if we shouldnt stop
		clr.w	HUD_Xvel.w				; stop moving
		addq.b	#1,HUD_State.w				; moving done
		bra.s	.render

.nostop
		move.w	HUD_Xvel.w,d0				; load x-velocity to d0
		ext.l	d0					; extend to long
		asl.l	#8,d0					; multiply by 8
		add.l	d0,HUD_Xpos.w				; add to x-position
		bra.s	.render
; ---------------------------------------------------------------------------
...
.getout
		...
		cmp.w	#$10,HUD_Xpos.w				; is HUD offscreen?
		bge.s	.render					; if not, just render
		clr.b	HUD_State.w				; reset HUD state
; ---------------------------------------------------------------------------

	; write mappings
.render
		move.w	HUD_Xpos.w,d0				; load x-pos to d0
		subq.w	#4,d7					; decrease piece count by 4

	; HP text
		move.w	#$90,(a4)+				; save y-pos
		move.b	#$0D,(a4)				; load the size of the sprite
		addq.w	#2,a4					; skip link parameter
	;	move.w	#$86C2,(a4)+				; load art address
		move.w	d1,(a4)+				; MJ: set pattern index address
		addq.w	#$08,d1					; MJ: advance index for next piece
		move.w	d0,(a4)+				; save x-pos

	; dripping blood
		move.w	#$A0,(a4)+				; save y-pos
		move.b	#$04,(a4)				; load the size of the sprite
		addq.w	#2,a4					; skip link parameter
	;	move.w	#$86CA,(a4)+				; load art address
		move.w	d1,(a4)+				; MJ: set pattern index address
		addq.w	#$02,d1					; MJ: advance index for next piece
		add.w	#$20,d0					; offset x-position
		move.w	d0,(a4)+				; save x-pos
	...
	; HP bar part 1
		...
		move.b	#$0D,(a4)				; load the size of the sprite
		addq.w	#2,a4					; skip link parameter
	;	move.w	#$86CC,(a4)+				; load art address
		move.w	d1,(a4)+				; MJ: set pattern index address
		addq.w	#$08,d1					; MJ: advance index for next piece
		move.w	d0,(a4)+				; save x-pos
	...
	; HP bar part 2
		move.w	#$90,(a4)+				; save y-pos
		move.b	#$09,(a4)				; load the size of the sprite
		addq.w	#2,a4					; skip link parameter
	;	move.w	#$86D4,(a4)+				; load art address
		move.w	d1,(a4)+				; MJ: set pattern index address
		addq.w	#$06,d1					; MJ: advance index for next piece
		add.w	#$20,d0					; offset x-position
		move.w	d0,(a4)+				; save x-pos
...
Render_Sprites:
		...
		beq.s	.NoHud					; MJ: if so, skip hud (no show)
		lea	(a6),a4					; MJ: use a4 (I need a6)
		move.w	#$8000|ArtTile_HUD,d1			; MJ: base index
		bsr.w	Render_HUD				; MJ: made it into a subroutine
		lea	(a4),a6					; MJ: restore back to a6
		bra.s	.NoCredits

	.NoHud:
		jsr	CRE_SpriteRender			; MJ: run credits sprites

	.NoCredits:
		bsr.w	Render_Rings

.xsprites
		move.l	ExtraSpriteRoutineFirst.w,a0		; load extra sprite routine to a0
		jsr	(a0)					; run it before rendering normal sprites
...
Render_Sprites_NextLevel:
		...
		jsr	DDZ_SpriteControl			; MJ: run sprite rendering routine

	.NoDDZ:

		move.l	ExtraSpriteRoutineLast.w,a0		; load extra sprite routine to a0
		jsr	(a0)					; run it before rendering normal sprites

		jsr	Render_SpriteLine			; MJ: render the sprite line during the credits
		jsr	CRE_SpriteClouds			; MJ: run credits cloud sprites
