; ===========================================================================
; ---------------------------------------------------------------------------
; Kosinski PLC direct decompression subroutine
; --- input -----------------------------------------------------------------
; a4.l = PLC list
; a1.l = RAM buffer to decompress art to and transfer from to VRAM
; ---------------------------------------------------------------------------

	; This routine is simply a crude replacement for the nemesis PLC
	; system, not meant to decompress during gameplay, instead decompresses
	; right away (to be used during init routines, and for game modes
	; which cannot afford the CPU and delay time).

KosDirectPLC:
		move.l	(a4)+,d0					; load art offset
		bmi.s	.Finish						; if there's nothing in the list, branch
		pea	(a1)						; store RAM buffer address

	.Next:
		movea.l	d0,a0						; set source address
		move.w	(a4)+,a2					; load VRAM address to transfer to
		move.l	(sp),a1						; load RAM buffer
		bsr.s	KosArt_To_VDP					; decompress art to RAM and then set the DMA request
		jsr	ProcessDMAQueue				; perform the DMA request
		move.l	(a4)+,d0					; load next offset
		bpl.s	.Next						; if not finished, branch
...
KosArt_To_VDP:
		...
		lsr.l	#$01,d1						; MJ: align for word count (because the subroutine no-longer does it)
