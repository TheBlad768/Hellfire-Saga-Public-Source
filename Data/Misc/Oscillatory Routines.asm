...
; Used for rings and giant rings
		...
		cmpi.b	#(ArtUnc_Ring_End-ArtUnc_Ring)>>8,(Rings_frame).w
		bne.s	.Sync
		clr.b	(Rings_frame).w
...
.Sync:
		...
		cmpi.b	#25,(v_rings).w							; NOV: Is Sonic's HP at max?
		blt.s	.SyncCont									; NOV: If so, branch
		move.l	#ArtUnc_RingTrans>>1,d1
...
.SyncCont:
		...
		move.w	#$100/2,d3								; Size of art (in words)	; We only need one frame
...
; NOV: Used for the HUD rings
.SyncHUD:
		lea	(HP_Ani_Frames).w,a0		; NOV: Get animation frames
		moveq	#5-1,d0					; NOV: 4 rings to animate
-
		tst.b	(a0,d0.w)				; NOV: Is this ring animating?
		beq.s	++						; NOV: If not, branch
		bpl.s	+						; NOV: If it's a loss animation, branch
		subq.b	#1,5(a0,d0.w)			; NOV: Decrement frame count
		bpl.s	++						; NOV: If it's not 0, branch
		addq.b	#1,(a0,d0.w)			; NOV: Increment frame
		move.b	#3,5(a0,d0.w)			; NOV: Reset frame delay
		st	(Update_HUD_ring_count).w	; NOV: Set to update art next frame
		bra.s	++						; NOV: Continue
+
		subq.b	#1,5(a0,d0.w)			; NOV: Decrement frame count
		bpl.s	+						; NOV: If it's not 0, branch
		subq.b	#1,(a0,d0.w)			; NOV: Decrement frame
		move.b	#3,5(a0,d0.w)			; NOV: Reset frame delay
		st	(Update_HUD_ring_count).w	; NOV: Set to update art next frame
+
		dbf	d0,-						; NOV: Loop until finished
