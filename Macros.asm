...
word_to_long function word1,word2,(((word1)<<16)&$FFFF0000)|((word2)&$FFFF)
...
; macro for generating data for MGZ slotted object entries. Using $460-$4F0 for the art.
SlotDataMGZ0	macro	vram, map, prio, width, height, frame, coll
	subObjSlotData	$03-1, $460|vram, $30, $00, map, prio, width, height, frame, coll
    endm

SlotDataSCZ0	macro	vram, map, prio, width, height, frame, coll
	subObjSlotData	$03-1, $42D|vram, $38, $00, map, prio, width, height, frame, coll
    endm
SlotDataSCZ1	macro	vram, map, prio, width, height, frame, coll
	subObjSlotData	$03-1, $441|vram, $38, $00, map, prio, width, height, frame, coll
    endm
; ---------------------------------------------------------------------------
...
music_noforce	macro track,terminate
		move.w	#track,d0
...
music		macro track,terminate
		...
		music_noforce	ALLARGS
	endm

command		macro track,terminate
		music_noforce	ALLARGS
	endm
...
samp		macro track,terminate
		music_noforce	ALLARGS
	endm


fadeToMusic	macro src
		jsr	Create_New_Sprite
		bne.s	.fail
		move.l	#Obj_Song_Fade_Transition,(a1)
		move.w	src,subtype(a1)
.fail
	endm
...
FREEZE	macro	Colour
	...
	.Loop:
	move.l	#(Colour<<$10)|Colour,($C00000).l
	dbf	d0,.Loop
	bra.w	*
	endm